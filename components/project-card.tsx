...
export function ProjectCard({ project, onAskAI, index = 0 }: { project: ProjectProps; onAskAI: (project: string) => void; index?: number }) {
    ...
    return (
        ...
        >
            ...
            {/* Inner Glow Border */}
            <div className="absolute inset-0 rounded-[2rem] ring-1 ring-white/10 group-hover:ring-primary/40 transition-all duration-500 z-20 pointer-events-none" />
            ...
            <div className="relative h-full p-8 flex flex-col z-30">
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                    {project.tech.map((t) => (
                        <span
                            key={t}
                            className="px-3 py-1 text-xs font-medium rounded-md bg-white/5 text-white border border-white/5 shadow-sm"
                        >
                            {t}
                        </span>
                    ))}
                ...
                <button
                    ...
                    className="w-full py-4 rounded-xl bg-gradient-to-r from-primary/10 to-accent/10 border border-white/10 hover:border-primary/50 text-white font-medium flex items-center justify-center gap-2 transition-all group-hover:shadow-[0_0_20px_rgba(123,44,191,0.2)]"
                >
